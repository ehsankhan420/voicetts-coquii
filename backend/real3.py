...
async def process_audio(audio_data, websocket):
    ...
    try:
        ...
        try:
            # Optimized TTS parameters for natural speech
            tts_model.tts_to_file(
                text=safe_response_text,
                speaker_wav=DEFAULT_SPEAKER_WAV,
                language=DEFAULT_LANGUAGE,
                file_path=temp_output_path,
                emotion="Happy",  # More expressive voice
                speed=1.0,       # Normal speaking speed
                split_sentences=True  # Better sentence handling
            )
